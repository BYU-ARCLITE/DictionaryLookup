...
object LangCodes {
  ...
  def ISO639toGoogle(code: String, source: Symbol) = {
    if (googleCodes.contains(code)) Some(code)
    else if(source == 'iso639_3) {
      ISO639_3to1.get(code)
        .filter(p1c => googleCodes.contains(p1c))
    } else if(source == 'iso639_1) {
      ISO639_1to3.get(code)
        .filter(p3c => googleCodes.contains(p3c))
    } else None
  }
  ...
  def convert(source: Symbol, target: Symbol, code: String) : Option[String] = {
    ...
    if(source == 'iso639_3) {
      target match {
        case 'iso639_1 => ISO639_3to1.get(code)
        case 'googleCodes => ISO639toGoogle(code, source)
        case _ => None
      }
    } else if(source == 'iso639_1) {
      target match {
        case 'iso639_3 => ISO639_1to3.get(code)
        case 'googleCodes => ISO639toGoogle(code, source)
        case _ => None
      }
    } else None
