...
object LookupMadamira extends Translator {
  ...
  def translate(user: User, src: String, dest: String, text: String): Option[Seq[String]] = {
      ...
      //val result = Http(madamiraURL).postData(inputXmlData).header("content-type", "application/xml").asString

      val query = WS.url(madamiraURL.getOrElse("https://127.0.0.1:8223"))
      ...
      if(result.status != 200) {
        play.Logger.debug("Madamira Code not OK(200).")
        None
      } else {
        play.Logger.debug("Madamira Code OK(200).")
        Some("Santiago is COOL@!#>"::parseXml(result.body))
      }
  }
}