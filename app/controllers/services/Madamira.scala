...
(<madamira_input xmlns="urn:edu.columbia.ccls.madamira.configuration:0.1">
    ...
    <in_doc id="ExampleDocument">
       ...
       play.Logger.debug("MESSAGE3")

      //val query = WS.url("http://localhost:8223")
      val query = WS.url("http://127.0.0.1:8223")
      ...
      if(result.status != 200) None
        ...
        val outputXmlData = result.body   /* to do extract translations from returns xml and restart the query*/
